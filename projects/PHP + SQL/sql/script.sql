create database if not exists bd_games default character set utf8mb4 collate utf8mb4_general_ci;
use bd_games;

create table if not exists usuários (
	usuário varchar(10) not null,
    nome varchar(30) not null,
    senha varchar(60) not null, /* O varchar(60) é por conta da criptografia da senha, que tem 60 caracteres */
...
create table if not exists gênero (
	cod int not null,
...
alter table jogos add column gênero int not null after nome;
alter table jogos add foreign key(gênero) references gênero(cod);

alter table jogos add column produtora int not null after gênero;
alter table jogos add foreign key(produtora) references produtoras(cod);
...
desc gênero;
insert into gênero values  (1, 'Ação'), (2, 'Aventura'), (3, 'Terror'), (4, 'Plataforma'), (5, 'Estratégia'), (6, 'RPG'), (7, 'Esporte'), (8, 'Corrida'), (9, 'Tabuleiro'), 
(10, 'Puzzle'), (11, 'Luta'), (12, 'Musical'), (13, 'Raciocínio');
select * from gênero;

desc produtoras;
insert into produtoras values (1, 'Microsoft', 'EUA'), (2, 'Tencent', 'China'), (3, 'Nintendo', 'Japão'), (4, 'Sony', 'Japão'), (5, 'Activision', 'EUA'), (6, 'EA', 'EUA'), 
(7, 'Sega', 'Japão'), (8, 'Namco Bandai', 'Japão'), (9, 'Konami', 'Japão'), (10, 'Ubisoft', 'EUA'), (11, 'Valve', 'EUA'), (12, 'Square Enix', 'Japão'), (13, 'Take Two', 'EUA'), 
(14, 'Capcom', 'Japão');
select * from produtoras;

desc jogos;
insert into jogos (cod, nome, gênero, produtora, descrição, nota, capa) values 
...
desc usuários;
insert into usuários (usuário, nome, senha, tipo) values
('admin', 'Leonardo Chiarelli', '$2y$10$UpMQCcir.v649HrdLvUXiOC/ftU7xWhSxm8QhX.VzSe9LZHhvW/Ty', 'admin'),
('teste', 'João da Silva', '$2y$10$w7on7cjLKNtmJUGkiHIXoOQAwTJzkgxXqLmjtfDUkCXmQK0784.IS', 'editor');
/*
	A senha de admin é 'admin', porém esta criptografada por PHP
    A senha de teste é 'teste'
*/
select * from usuários;

select j.cod, j.nome, g.gênero, j.descrição, j.nota, j.capa from jogos as j
join gênero as g on j.gênero = g.cod; /* Mostra apenas o gênero */

select j.cod, j.nome, p.produtora, j.descrição, j.nota, j.capa from jogos as j
join produtoras as p on j.produtora = p.cod; /* Mostra apenas a produtora */

/* CÓDIGO QUE SERÁ USADO NO PHP */ 
select j.cod, j.nome, g.gênero, p.produtora, j.descrição, j.nota, j.capa from jogos as j
join gênero as g on j.gênero = g.cod
join produtoras as p on j.produtora = p.cod; /* Mostra gênero e produtora */